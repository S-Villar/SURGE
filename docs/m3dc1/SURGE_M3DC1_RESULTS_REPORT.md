...
_Status snapshot: **2026‑07‑03**. Repo: **SURGE-exp**, branch `feat/m3dc1-on-model-bench`._
...
### 4.5 High‑m tail, wide m‑window, and geometry channels (2026‑07‑03)

Many edge‑localized modes carry a non‑negligible fraction of their energy at **native
m > 20**, outside the baseline training window **m ∈ [−80, 20]**. A full‑dataset scan
([`scan_m_hi_tail.py`](../../scripts/m3dc1/internal/scan_m_hi_tail.py)) quantifies this:

![high-m tail scan](assets/m_hi_tail_scan.png)

| Native energy at **m > 20** | Cases | Share |
|---|---:|---:|
| ≥ 1% | 3,354 | 34% |
| ≥ 5% | 199 | **2%** |
| ≥ 10% | 109 | 1% |

The median case has only **0.75%** energy at m > 20 — the baseline window is fine for
typical cases. The tail matters for **~200 edge cases** (≥ 5% high‑m energy), which are
**75–90% edge‑localized** (peak ψ_N > 0.9).

#### Wide symmetric window `[-100, 100]` — global vs tail trade‑off

| Run | m window | grid | test global R² | test pattern R² |
|---|---|---:|---:|---:|
| **`spectrum_fno48_floor6_smooth1_qc`** | [−80, 20] | 128 | **0.920** | **0.924** |
| `spectrum_fno48_floor6_smooth1_qc_m100` | [−100, 100] | 128 | 0.890 | 0.906 |
| `spectrum_fno48_floor6_smooth1_qc_m100gr192` | [−100, 100] | 192 | 0.886 | **0.894** |
| `spectrum_fno48_floor6_smooth1_qc_geom` | [−80, 20] + geom | 128 | 0.919 | 0.925 |

**Globally, wider windows hurt** — same 75M‑parameter FNO capacity is spread over twice
the m‑range with many extra noise‑floor pixels.

**On the high‑m tail subset (test cases with ≥ 5% native energy at m > 20, n = 40):**

| Model | pattern R² (full image) | pattern R² **m > 20 only** | pattern R² **m ∈ [−80, 20]** (fair core band) | Δ vs `_qc` on core band |
|---|---:|---:|---:|---:|
| `_qc` baseline | 0.855 | *(not predicted)* | 0.855 | — |
| **`m100g128`** | **0.909** | **0.910** | **0.879** | **+0.025** (wins 88%) |
| `m100gr192` | 0.892 | 0.889 | 0.858 | +0.003 (wins 58%) |

**Verdict on `m100gr192`:** it does **not** beat `m100g128` on high‑m tail cases. Despite
finer ψ/m sampling (grid 192), aggregate pattern R² is lower (0.894 vs 0.906) and the
dedicated **m > 20** region R² on tail≥5% cases is **0.889 vs 0.910** for grid 128. On
the fair core band m ∈ [−80, 20], `m100gr192` is essentially tied with `_qc` (+0.003),
while **`m100g128` gains +0.025** on the same band — widening the window helps the tail
only at grid 128, not 192.

#### Error vs high‑m tail fraction

Per‑case pattern R² degrades as the native high‑m energy fraction grows (test split).
Scatter + binned means ([`plot_hi_m_tail_scatter.py`](../../scripts/m3dc1/internal/plot_hi_m_tail_scatter.py)):

![hi-m tail R2 scatter](assets/hi_m_tail_r2_scatter_qc_unet.png)

For `_qc`, the 2–5% tail bin already drops to R² ≈ 0.83; cases with ≥ 20% energy at
m > 20 average R² ≈ 0.84. U‑Net follows the same trend. A full multi‑model scatter
(including `m100g128`, `m100gr192`, `_qc_geom`) is generated by
[`generate_mwindow_assets.slurm`](../../scripts/m3dc1/internal/generate_mwindow_assets.slurm).

#### Metric reality‑check galleries — native order vs matched `_qc` cases

Each model gets two combined spectrum+field galleries (6 panels per row):
1. **Native** — cases p2…p98 worst→best *for that model's* pattern R².
2. **Reference‑matched** — the *same case keys* at the same percentiles from `_qc`, so
   cross‑model rows are directly comparable.

| Gallery | Description |
|---|---|
| [`metric_reality_check_qc_combined.png`](assets/metric_reality_check_qc_combined.png) | `_qc` baseline (native order) |
| [`metric_reality_check_unet_combined.png`](assets/metric_reality_check_unet_combined.png) | U‑Net (native) |
| [`metric_reality_check_unet_refqc_combined.png`](assets/metric_reality_check_unet_refqc_combined.png) | U‑Net on `_qc` p2…p98 cases |
| `metric_reality_check_m100g128_combined.png` | wide window g128 (native) — *generating* |
| `metric_reality_check_m100g128_refqc_combined.png` | g128 on `_qc` cases — *generating* |
| `metric_reality_check_m100g192_refqc_combined.png` | g192 on `_qc` cases — *generating* |
| `metric_reality_check_qc_geom_combined.png` | geometry channels (native) — *generating* |

Generate with:
```bash
python scripts/m3dc1/internal/metric_gallery.py \
    --cache runs/spectrum_fno48_floor6_smooth1_qc_m100/predictions_cache.npz \
    --ref-cache runs/spectrum_fno48_floor6_smooth1_qc/predictions_cache.npz \
    --out docs/m3dc1/assets/metric_reality_check_m100g128_refqc_combined.png
```

#### `--geom-channels` — what it adds

With `--geom-channels`, three **ψ_N‑only** input planes are appended (repeated across m
rows), computed from each case's equilibrium HDF5 in
[`_geometry_profiles()`](../../scripts/m3dc1/internal/train_spectrum_image.py):

| Channel | Formula / meaning |
|---|---|
| `shear` | `(ψ_N / q) · dq/dψ_N` — magnetic shear proxy |
| `flux_exp` | flux‑surface‑averaged `\|∇ψ\|` binned by ψ_N, normalized — edge flux compression |
| `lcfs_prox` | `1 − ψ_N` (1 at axis, 0 at LCFS) |

These give the FNO explicit edge‑geometry cues that ψ_N alone does not encode (real‑space
LCFS shape, grad‑ψ compression at the pedestal). **`_qc_geom`** reaches test pattern R²
**0.925** vs **0.924** for `_qc` at the same m window — a small aggregate gain. It does
**not** add m > 20 rows; high‑m tail cases still need a wider or asymmetric m window.

#### Should we try `fno_modes=96` or `fno_hidden=64`?

Probably **not as the next lever** on grid 128:

| Config | Params | Notes |
|---|---:|---|
| modes=48, hidden=32 (current) | **75M** | modes=48 already spans to FFT Nyquist on grid 128 |
| modes=96, hidden=32 | ~300M | 4× spectral weights; dim returns, not new physics |
| modes=48, hidden=64 | ~150M | wider conv trunk; doesn't fix m‑window coarsening |
| grid=192, modes=48 | 75M | **tested** — worse global *and* worse high‑m tail vs g128 |

The bottleneck on edge/high‑m modes is **missing input coverage** (m > 20 not in the
target) and **field‑space sharpness**, not FNO spectral truncation at 48 modes.
Recommended next experiment: **asymmetric window `[−80, 100]`** at grid 128 (keep full
resolution in the trained band, add high‑m rows only where needed) plus geometry channels.
...
**Known limitation — geometry is not an input (baseline `_qc`).** Training happens
entirely in `(m, ψ_N)` spectral space. The model sees the `ψ_N` coordinate (so ψ_N=1 =
LCFS), `q`/`p` profiles, the `m−n·q` resonance, and shaping scalars (κ, δ, R0, a) — but
**not** the real‑space `ψ(R,Z)` flux map, edge flux‑surface compression, or the LCFS `(R,Z)`
location. Edge‑localized modes' sharpness/`m`‑broadening are set by exactly that edge
geometry, which is the population the model handles worst. **`--geom-channels`** (§4.5)
addresses part of this; high‑m tail cases additionally need m‑window coverage.

**Next steps:** (1) **asymmetric m window `[−80, 100]`** at grid 128; (2) combine with
`--geom-channels`; (3) attack peak amplitude with `--peak-weight` and reduced smoothing
(target peak RMSE < 0.2 dex so edge modes survive field reconstruction); (4) FNO+U‑Net
ensemble; (5) physics‑residual target (predict deviation from the `m ≈ n·q(ψ)` ridge
prior); (6) correlate residual error with `gs_error`/γ(t) convergence.

---

## 7. Field-based model selection (pre-field-loss baseline) — 2026-07-04

We added [`field_bench.py`](../../scripts/m3dc1/internal/field_bench.py) to rank spectrum
surrogates by **post-hoc field relL2** (true phase injected, IFFT, max-normalized —
identical convention to [`field_recon_compare.py`](../../scripts/m3dc1/internal/field_recon_compare.py)),
not by spectrum pattern R². Full test split (**n = 1994**).

### 7.1 Leaderboard (test split, all cases)

| Model | frac(relL2>1) ↓ | p90 relL2 ↓ | median | mean | mean α-opt | CRF ↑ |
|---|---:|---:|---:|---:|---:|---:|
| **`qc_peak4`** | **0.180** | **1.165** | **0.730** | **0.808** | **0.696** | **0.825** |
...
Artifacts: [`field_bench/leaderboard.json`](../../field_bench/leaderboard.json),
[`per_case.csv`](../../field_bench/per_case.csv),
[`per_family.csv`](../../field_bench/per_family.csv),
`relL2_hist_*.png`.

### 7.2 Interpretation (data-backed)

- **Amplitude error is a small fraction of relL2.** α-optimal relL2 (scalar rescale before
  relL2) drops mean relL2 by only **~0.11–0.15** for all three models (e.g. peak4 mean
  0.808 → 0.696). Floor-5 experiments are **deprioritized** — tightening the log floor
  did not fix field fidelity.
- **Dominant error is structural/coherent.** High CRF (peak4 **0.825** vs `_qc` **0.737**)
  means most residual energy sits in low radial-wavenumber bands — errors that survive
  field reconstruction, not incoherent speckle.
- **Pattern R² overstates deployment quality** (§4.4, floor5 counterexample). Model
  selection must use field metrics, not patR².

### 7.3 Next experiment (field-loss training)

Single lever on **`qc_peak4`** recipe (m ∈ [−80, 20], `--peak-weight 4`, floor6,
smooth1, quarantine, **no geom**):

1. Add differentiable **IFFT-proxy field loss** with true phase (oracle phase idealization).
2. Select checkpoints by **`frac(relL2>1)` then p90** on a family-stratified val subset
   (`--select-by field`), not patR².
3. **4 h interactive** A100 sessions only; resumable with persisted early-stop state
   (fix resume resetting patience — see §8).

---

## 8. Resume / early-stop bug (qc_mhi100)

`qc_mhi100` training was killed at epoch **237** (4 h wall). On `--resume`, the early-stop
