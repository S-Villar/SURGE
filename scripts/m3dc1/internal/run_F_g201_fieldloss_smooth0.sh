#!/usr/bin/env bash
# Run F: native training grid 201x201 + D' recipe — closes pixelation / resolution question.
# FNO modes scaled up (~50% of Nyquist on 201 grid). Reduce batch for VRAM.
set -euo pipefail
...
export OUT="${OUT:-runs/spectrum_fno48_floor6_smooth0_qc_peak4_fieldloss_g201}"
mkdir -p "$OUT"
RESUME=()
