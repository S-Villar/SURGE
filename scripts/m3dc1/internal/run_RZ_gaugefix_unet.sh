#!/usr/bin/env bash
# Gauge-fixed RZ — U-Net backend (3a). Single lever: --models unet
set -euo pipefail
cd /global/homes/a/asvillar/src/SURGE
source scripts/m3dc1/surge_slurm_env.sh && surge_slurm_setup_python
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

OUT="${OUT:-runs/rz_field_gaugefix_unet}"
mkdir -p "$OUT"
RESUME=()
[[ -f "$OUT/ckpt_unet_last.pt" ]] && RESUME=(--resume "$OUT/ckpt_unet_last.pt")
...
python -u scripts/m3dc1/internal/train_rz_field_image.py \
  ...
  --models unet --fno-modes 64 --fno-hidden 32 \
