...
from .pytorch_impl import PyTorchMLPModel, PyTorchCNNModel
...
_TORCH_CNN_PROFILE = ResourceProfile(
    name="pytorch.cnn",
    supports_cpu=True,
    supports_gpu=True,
    worker_semantics="dataloader_workers",
...
class PyTorchCNNAdapter(BaseModelAdapter):
    """PyTorch-based CNN model for parameter-to-image prediction."""
    ...
    @property
    def training_history(self):
        """Per-epoch loss/RMSE records (PyTorch CNN); surfaced to the engine for artifacts."""
        inner = getattr(self, "_model", None)
        if inner is None or getattr(inner, "model", None) is None:
            return None
        return getattr(inner.model, "training_history", None)
