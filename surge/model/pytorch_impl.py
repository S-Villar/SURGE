...
class PyTorchCNN(nn.Module if TORCH_AVAILABLE else object):
    """
    PyTorch CNN for parameter-to-image prediction using MLP encoder + transposed convolutions.

    Architecture:
    - MLP encoder: input_size -> hidden_layers -> latent_dim
    - Decoder: latent_dim -> transposed conv layers -> output_shape
    """

    def __init__(self, input_size, hidden_layers, output_shape=(28, 28),
                 dropout_rate=0.1, activation_fn=None, learning_rate=1e-3, n_epochs=300,
                 batch_size=32, device=None, dataloader_num_workers: int = 0,
                 pin_memory: Optional[bool] = None,
                 inference_batch_size: Optional[int] = None,
                 log_progress: bool = False,
                 ...
                 latent_dim: int = 128):
        if not TORCH_AVAILABLE:
            raise ImportError("PyTorch is required for PyTorchCNN. Install with: pip install torch")
        if activation_fn is None:
            activation_fn = nn.ReLU
        ...
        self.output_size = output_shape[0] * output_shape[1]  # flattened size
        self.learning_rate = learning_rate
        self.n_epochs = n_epochs
        self.batch_size = batch_size
        self.dataloader_num_workers = int(dataloader_num_workers)
        ...
        # Build transposed CNN decoder
        # Start from latent_dim and upsample to output_shape
        self.decoder = nn.Sequential(
            # Reshape latent to feature maps
            nn.Unflatten(1, (latent_dim, 1, 1)),  # (latent_dim, 1, 1)

            # Transposed conv layers to upsample
            nn.ConvTranspose2d(latent_dim, 128, kernel_size=7, stride=4, padding=0),  # -> (128, 7, 7)
            activation_fn(),
    ...
    def forward(self, x):
        ...
        return reconstructed.view(reconstructed.size(0), -1)
    ...
    def fit(self, X, y, X_val=None, y_val=None, patience=None, finetune=False):
        ...
        if y.ndim == 2 and y.shape[1] == self.output_size:
            ...
            y_flat = y.reshape(-1, self.output_size) if y.ndim == 1 else y
        ...
        if use_early_stop:
            ...
            if isinstance(y_val, torch.Tensor):
                y_val = y_val.cpu().numpy()
            if y_val.ndim == 3:  # (N, H, W)
                y_val_flat = y_val.reshape(y_val.shape[0], -1)
            elif y_val.ndim == 4:  # (N, C, H, W)
                y_val_flat = y_val.reshape(y_val.shape[0], -1)
            else:
                y_val_flat = y_val.reshape(-1, self.output_size) if y_val.ndim == 1 else y_val
            if finetune:
                X_val_scaled = X_val
                y_val_scaled = y_val_flat
            else:
                X_val_scaled = self.scaler_X.transform(X_val)
                y_val_scaled = self.scaler_y.transform(y_val_flat)
            X_val_cpu = torch.tensor(X_val_scaled, dtype=torch.float32)
            y_val_cpu = torch.tensor(y_val_scaled, dtype=torch.float32)
            val_loader = _make_dataloader(
                TensorDataset(X_val_cpu, y_val_cpu),
        ...
        for epoch in range(self.n_epochs):
            ...
            if use_early_stop:
                ...
                if self.log_progress:
                    _LOGGER.info(
                        "[torch.cnn] epoch %d/%d | train_RMSE(scaled)=%.6f | "
                        "val_RMSE(scaled)=%.6f | train_rows=%d/%d | patience=%d/%d",
                        epoch + 1,
                        self.n_epochs,
                        train_rmse,
                        val_rmse,
        ...
        if (
            ...
            self.load_state_dict(device_state)
            _LOGGER.info(
                "[torch.cnn] restored weights from best validation (epoch %d / val_loss=%.8f)",
                best_epoch_num,
                best_val_loss,
            )
    ...
    def predict(self, X):
        ...
        # Reshape to image format (N, H*W) -> (N, 1, H, W)
        if y_pred.shape[1] == self.output_size:
            H, W = self.output_shape
            y_pred = y_pred.reshape(-1, 1, H, W)  # Add channel dimension
...
class PyTorchCNNModel:
    """
    SURGE-compatible wrapper for PyTorch CNN models.
    Supports early stopping via patience (epochs without val improvement).
    """

    def __init__(self, hidden_layers=[256, 128], dropout_rate=0.1, activation_fn="relu",
                 learning_rate=1e-3, n_epochs=300, batch_size=32, patience=None,
                 max_epochs=None, epochs=None, dropout=0.1,
                 dataloader_num_workers: int = 0, pin_memory=None,
                 inference_batch_size=None, log_progress: bool = False,
                 ...
                 checkpoint_every_n_epochs: int = 0,
                 restore_best_weights: bool = True,
                 latent_dim: int = 128, output_shape=(28, 28)):
        if not TORCH_AVAILABLE:
            raise ImportError("PyTorch is required for PyTorchCNNModel. Install with: pip install torch")
    ...
    def load(self, filepath):
        ...
        self.output_shape = config.get("output_shape", (28, 28))
        self.model = PyTorchCNN(**config)
        self.model.load_state_dict(checkpoint['model_state_dict'])
        self.model.scaler_X = checkpoint['scaler_X']
        self.model.scaler_y = checkpoint['scaler_y']
        if isinstance(checkpoint.get("training_history"), list):
